...
class CRNN(torch.nn.Module):
    ...
    def ttraining(self,epok = 0):
        ...
        for epoch in tqdm(range(self.num_epochs), desc="Training"):
            ...
            # Log metrics
            with open("accuracy_x2v3.csv", "a") as acc:
            ...
            torch.save(self.state_dict(), os.path.join(option.PathToLetterNet, f"netCRNNX2v3_{epoch + epok}.pt"))
            ...
            # Plot loss
            df = pd.read_csv("accuracy_x2v3.csv")
        ...
        with torch.no_grad():
            for x_val, y_val in tqdm(val_loader):
                ...
                for label in y_val:
                    chars = [self.symbols[i.item()] for i in label if i.item() != self.num_classes]
                    decoded_targets.append("".join(chars))
                [print(f"{i == j} pred: {i}, target: {j}") for i, j in zip(decoded_preds, decoded_targets)]
...
if __name__ == "__main__":
    ...
    model = CRNN()
    #model.load_state_dict(torch.load(os.path.join(option.PathToLetterNet, f"netCRNNX2v3_0.pt")))
    ...
    #model.device = "cpu"
    # Train the model
    #print(model.run(r"A:\Overførsler\download.jpg"))
    model.ttraining()
