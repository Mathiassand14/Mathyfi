...
def inkml2img(input_path: str, output_path: str, color='black') -> None:
    ...
    for elem in traces:
        ...
        for subls in ls:
            data = np.array(subls)
            #print(data)
            #remove if point is wrong
            #dat = np.array([[i[0], i[1]] for i in data])
            #median = np.median(dat[:, 0], axis=0), np.median(dat[:, 1], axis=0)
            #abs_dev = np.abs(dat[:, 0] - median[0]), np.abs(dat[:, 1] - median[1])
            #mad = np.median(abs_dev[0], axis=0), np.median(abs_dev[1], axis=0)
            #mod_z = 0.6745 * abs_dev[0] / mad[0], 0.6745 * abs_dev[1] / mad[1]
            #dat = np.array([d for i, d in  enumerate(dat) if mod_z[0][i] < 3 and mod_z[1][i] < 3])
            ##dat = np.array([[i[0], i[1]] for i in data])
            #if len(dat) < 1 or len(dat[0]) != 2:
            #    continue
            #print(dat)
            x, y, _ = zip(*data)
